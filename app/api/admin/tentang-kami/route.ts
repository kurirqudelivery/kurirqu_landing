...
// GET - Fetch Tentang Kami content
export async function GET() {
  try {
    ...
    console.error('Error fetching Tentang Kami content:', error)
    return NextResponse.json(
      { error: 'Failed to fetch Tentang Kami content' },
      { status: 500 }
    )
  }
}

// PUT - Update Tentang Kami content
export async function PUT(request: NextRequest) {
  try {
    ...
    await contentModel.updateTentangKamiContent(data)
    
    return NextResponse.json({ success: true, message: 'Tentang Kami content updated successfully' })
  } catch (error) {
    console.error('Error updating Tentang Kami content:', error)
    return NextResponse.json(
      { error: 'Failed to update Tentang Kami content' },
      { status: 500 }
    )
  }
}
