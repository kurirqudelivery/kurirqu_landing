  ...
  try {
    ...
    for (const [templateKey, templateData] of Object.entries(templates)) {
      try {
        switch (templateKey) {
          ...
          case 'services':
            ...
            await db.collection('services').insertMany(
              (templateData as any[]).map((service: any, index: number) => ({
                ...service,
                order: index,
                createdAt: new Date(),
                updatedAt: new Date()
          ...
          case 'testimonials':
            ...
            await db.collection('testimonials').insertMany(
              (templateData as any[]).map((testimonial: any, index: number) => ({
                ...testimonial,
                order: index,
                createdAt: new Date(),
                updatedAt: new Date()
          ...
          case 'gallery':
            ...
            await db.collection('gallery').insertMany(
              (templateData as any[]).map((item: any, index: number) => ({
                ...item,
                order: index,
                createdAt: new Date(),
                updatedAt: new Date()
          ...
          case 'stats':
            ...
            await db.collection('stats').insertMany(
              (templateData as any[]).map((stat: any, index: number) => ({
                ...stat,
                order: index,
                createdAt: new Date(),
                updatedAt: new Date()
          ...
          case 'cta':
            await db.collection('ctas').deleteMany({})
          ...
          case 'footer':
            await db.collection('footers').deleteMany({})
          ...
          case 'tentang-kami':
            await db.collection('tentangkami').deleteMany({})
          ...
          case 'syarat-ketentuan':
            await db.collection('syaratketentuan').deleteMany({})
          ...
          case 'kebijakan-privasi':
            await db.collection('kebijakanprivasi').deleteMany({})
        ...
        console.error(`Error seeding ${templateKey}:`, error)
      }
    }

    return NextResponse.json({ 
      message: `Semua template berhasil di-seed!`,
      seededTemplates: results,
      totalSeeded: results.length
    })

  } catch (error) {
    console.error('Seed all templates error:', error)
    return NextResponse.json({ 
      error: 'Gagal melakukan seed semua template' 
    }, { status: 500 })
  }
}
