  ...
  try {
    ...
    for (const [templateKey, templateData] of Object.entries(templates)) {
      try {
        ...
        console.error(`Error restoring ${templateKey}:`, error)
      }
    }

    return NextResponse.json({ 
      message: `Semua template berhasil di-restore!`,
      restoredTemplates: results,
      totalRestored: results.length
    })

  } catch (error) {
    console.error('Restore all templates error:', error)
    return NextResponse.json({ 
      error: 'Gagal melakukan restore semua template' 
    }, { status: 500 })
  }
}
